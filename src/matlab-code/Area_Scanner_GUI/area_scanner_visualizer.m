...
% Define the Unity server address and port
unity_server_address = '127.0.0.1';
unity_server_port = 12345;

% Create a UDP object for communication
udp_client = udp(unity_server_address, unity_server_port, 'LocalPort', 0);
            ...
            if(SHOW_TRACKED_OBJ)
                if(HAVE_VALID_TARGET_LIST)
                    ...
                    if(numTargets > 0)
                        ...
                        % SEND TRACKED OBJS TO UNITY SERVER
                        data_to_send = [newframe.targets.posX; newframe.targets.posY; newframe.targets.posZ;];
                        fwrite(udp_client, mat2str(data_to_send));
